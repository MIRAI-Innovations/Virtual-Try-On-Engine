...
// --- LOGGING ---
const LOG_FILE = './mirai_server.log';
...
app.use((req, res, next) => {
    logToFile(`[DEBUG] Incoming Request: ${req.method} ${req.path}`);
    next();
});
...
app.get('/api/garments', (req, res) => {
    logToFile("[Request] Fetching garments list");
    try {
        ...
        logToFile(`Error listing garments: ${e}`);
...
app.post('/recommend', (req, res) => {
    logToFile("[Request] Received Recommendation request");
...
app.post('/api/tryon', (req, res) => {
    logToFile(`[Request] Received VTON request (Mode: ${VTON_MODE})`);
    ...
    try {
        ...
        logToFile(`[IO] Saved input image to: ${inputPath}`);
        ...
        if (!fs.existsSync(garmentPath)) {
            logToFile(`[Warning] Garment file not found locally: ${garmentPath}`);
        }

        // Map frontend categories to Python engine accepted choices
        let engineCategory = category || 'Upper-body';
        if (engineCategory === 'T-SHIRTS' || engineCategory === 'Recommended' || engineCategory === 'for-you') {
            engineCategory = 'Upper-body';
        }
        ...
        logToFile("[Process] Spawning Python engine...");
        ...
        pythonProcess.stderr.on('data', (data) => {
            ...
            logToFile(`[Python Error]: ${data}`);
        });

        pythonProcess.on('close', (code) => {
            logToFile(`[Process] Python exited with code ${code}`);
            ...
            if (match && match[1]) {
                try {
                    ...
                    if (outputData.success && outputData.result_path) {
                        ...
                        logToFile(`[Success] Found result at: ${resultPath}`);
                    ...
                    logToFile(`[Error] Failed to parse Python JSON output: ${e}`);
                }
            }

            logToFile("[Error] No valid result found in python output");
            // Debugging: Print last few lines of output
            logToFile(`Partial Output: ${dataString.slice(-500)}`);
            return res.status(500).json({ error: "No result generated", details: errorString });
        });

    } catch (e) {
        logToFile(`Server Error: ${e}`);
        return res.status(500).json({ error: "Internal Server Error" });
    }
});

app.listen(PORT, () => {
    logToFile(`MIRAI VTON Server is listening on port ${PORT}...`);
    logToFile(`Mode: ${VTON_MODE.toUpperCase()} | Provider: ${USE_PAID_API ? 'SEGMIND (PAID)' : 'GRADIO (FREE)'}`);
});
